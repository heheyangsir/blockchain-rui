  ...
  <div class="h-screen bg-gradient-to-b from-orange-50 to-gray-200 flex flex-col">
    ...
    <main class="flex-1 overflow-y-auto pt-24 pb-16">
      <div class="max-w-5xl w-full mx-auto px-6 space-y-10">
        ...
        <!-- 上传表单 -->
        <section class="bg-white p-8 rounded-2xl shadow-xl border border-gray-200 space-y-6">
          <!-- 凭证名称 -->
          <div>
            <label class="block text-gray-700 font-semibold mb-2">📛 凭证名称</label>
            <input v-model="credentialName" placeholder="例如：区块链课程证书"
          ...
          <!-- 上传按钮 -->
          <div>
            <button @click="storeCredential" class="w-full bg-gradient-to-r from-green-500 to-green-600 
          ...
          <!-- 交易哈希 -->
          <p v-if="transactionHash" class="text-green-700 bg-green-100 p-4 rounded-lg text-sm break-words shadow">
            ✅ 交易哈希：{{ transactionHash }}
          </p>
        </section>

        <!-- 错误信息 -->
        <p v-if="error" class="bg-red-100 text-red-700 p-4 rounded-lg text-sm text-center shadow">
          ⚠️ {{ error }}
        </p>
      </div>
    </main>
...
const connect = () => {
  ...
  account.value = accountStore.accountAddress!
